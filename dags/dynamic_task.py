...
class DbtDagParser:
    ...
    def initialize_task_groups(self):
        """Initialize TaskGroups and populate them."""
        self.dbt_run_group = TaskGroup('dbt_run', dag=self.dag)
        self.dbt_test_group = TaskGroup('dbt_test', dag=self.dag)
    ...
    def load_dbt_manifest(self):
        ...
        """
        manifest_path = os.path.join(self.dbt_manifest, "manifest.json")
    ...
    def make_dbt_task(self, node_name, dbt_verb):
        ...
        execution_date_dict = {"execution_date": self.execution_date}
